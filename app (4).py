...
"""
    ...
    if st.session_state.ai_loading and st.session_state.chat_history:
        with st.spinner("🤖 AI sedang menganalisis..."):
            try:
                ...
                # Hanya kirim pesan user (bukan assistant) untuk menghindari
                # pesan terakhir dari assistant yang bisa muncul dobel
                messages_payload = []
                ...
                if "NO_KEY" in str(e):
                    fallback = (
                        ...
                        "Pastikan format di Streamlit Secrets sudah benar:\n"
                        "```\nANTHROPIC_API_KEY = \"sk-ant-...\"\n```\n"
                        "Setelah disimpan, klik **Reboot app** di Streamlit Cloud."
                    )
                else:
                    fallback = f"❌ Error: {str(e)[:150]}"
                ...
                if e.code == 401:
                    fallback = "❌ **API key tidak valid (401 Unauthorized).** Periksa kembali nilai key di Streamlit Secrets — pastikan tidak ada spasi atau karakter ekstra."
                elif e.code == 403:
                    fallback = "❌ **Akses ditolak (403 Forbidden).** Periksa apakah API key sudah aktif di console.anthropic.com."
                elif e.code == 429:
                    fallback = "⏳ **Rate limit tercapai.** Tunggu sebentar lalu coba lagi."
                else:
                    fallback = f"❌ HTTP Error {e.code}: {body[:200]}"
                st.session_state.chat_history.append({"role": "assistant", "content": fallback})
                st.session_state.ai_loading = False
                st.rerun()
                ...
                if "timeout" in err_msg.lower():
                    ...
                    fallback = f"❌ Terjadi error tidak terduga: {err_msg[:200]}"
                st.session_state.chat_history.append({"role": "assistant", "content": fallback})
                st.session_state.ai_loading = False
                st.rerun()
    ...
    if not st.session_state.chat_history:
        ...
        <div style='background:rgba(201,168,76,.05);border:1px solid rgba(201,168,76,.15);
          ...
          <div style='color:#c9a84c;font-weight:700;font-size:.875rem;margin-bottom:.75rem'>⚙️ Cara Mengaktifkan AI</div>
          <div style='color:#7a92a8;font-size:.83rem;line-height:2.1'>
            <strong style='color:#e8f0f8'>1.</strong> Dapatkan API key di
              <strong style='color:#2dd4bf'>console.anthropic.com</strong> → API Keys → Create Key<br>
            <strong style='color:#e8f0f8'>2.</strong> Di Streamlit Cloud → buka app → klik
              <strong style='color:#e8f0f8'>⋮ (titik tiga)</strong> → <strong style='color:#e8f0f8'>Settings → Secrets</strong><br>
            <strong style='color:#e8f0f8'>3.</strong> Paste <em>persis seperti ini</em> (tanpa spasi ekstra):<br>
          </div>
          <div style='background:#0c1422;border:1px solid #1a2c3d;border-radius:8px;
                      padding:.75rem 1rem;margin:.6rem 0;font-family:monospace;font-size:.85rem;color:#2dd4bf'>
            ANTHROPIC_API_KEY = "sk-ant-api03-..."
          </div>
          <div style='color:#7a92a8;font-size:.83rem;line-height:2.1'>
            <strong style='color:#e8f0f8'>4.</strong> Klik <strong style='color:#e8f0f8'>Save</strong>
              lalu klik <strong style='color:#e8f0f8'>Reboot app</strong><br>
            <strong style='color:#e8f0f8'>5.</strong> Kembali ke halaman ini dan coba kirim pertanyaan ✅
          </div>
        </div>
        """, unsafe_allow_html=True)
